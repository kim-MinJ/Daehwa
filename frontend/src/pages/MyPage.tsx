    ...
    <div className="space-y-4">
      {comments.map((c) => (
        <div key={c.commentIdx} className="border rounded-lg p-4 bg-gray-50 shadow-sm">
          <div className="flex justify-between items-center mb-2">
            <h3 className="font-semibold">{c.movie?.title}에 단 댓글</h3>
            <div className="text-sm text-gray-500 flex flex-col">
              <span>작성일: {new Date(c.createdAt).toLocaleDateString()}</span>
              <span>
                수정일:{" "}
                {c.updateAt
                  ? new Date(c.updateAt).toLocaleDateString()
  ...
  variant="outline"
  onClick={async () => {
    // 댓글 데이터가 없으면 먼저 fetch
    try {
      const res = await axios.get(`/api/review/${c.reviewIdx}/comments`, { headers: authHeader });
