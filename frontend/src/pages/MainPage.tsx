...
import { ChevronRight, Info, Star, MessageSquare } from "lucide-react";
...
// ID 중복 제거 + 포스터 없는 항목 제외
const uniqueByIdAndPoster = (movies: UiMovie[]): UiMovie[] => {
  const seen = new Set<string | number>();
  return movies.filter((movie) => {
    if (!movie.poster) return false; // 포스터 없으면 제외
    ...
    (async () => {
      try {
        const [latestResp, ratingResp] = await Promise.all([
          fetchMovies({ q: "", page: 1, sort: "latest" }, token ?? undefined),
          fetchMovies({ q: "", page: 1, sort: "rating" }, token ?? undefined),
        ]);
        ...
        setLatest(uniqueByIdAndPoster(latestList.map(mapApiToUi)));
        setTopRated(uniqueByIdAndPoster(ratingList.map(mapApiToUi)));
      } catch (e) {
      ...
      <main className="relative">
        ...
        {featured && (
          <div className="relative h-[85vh] mb-8">
            ...
            <div className="absolute bottom-0 left-0 w-full max-w-7xl mx-auto px-8 lg:px-16 pb-8 lg:pb-16">
              <h1 className="text-5xl lg:text-7xl font-bold text-white mb-6">{featured.title}</h1>
              {featured.description && (
                <p className="text-white/90 text-lg lg:text-xl mb-6">{featured.description.slice(0, 200)}...</p>
              )}
            </div>
          </div>
        )}
        ...
        {/* 맞춤 추천 TOP3 */}
        <section className="max-w-7xl mx-auto px-8 lg:px-16 mb-16">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent">
                    <div className="absolute bottom-4 left-4 right-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <Badge className="bg-white/20 text-white text-xs">맞춤 추천</Badge>
                      </div>
                ...
                <div className="w-48 aspect-[2/3] rounded-lg overflow-hidden relative transition-transform duration-300 group-hover:scale-105">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="absolute bottom-3 left-3 right-3">
                      <h4 className="text-white font-bold text-sm line-clamp-1">{movie.title}</h4>
                    </div>
                  </div>
                </div>
              </div>
