...
export default function AdminVotesTab({
  ...
  const getStatusBadge = (status: string) => {
    ...
    return <span className={`${colorMap[status as keyof typeof colorMap]} text-white px-2 py-1 rounded`}>{status}</span>;
  };
              ...
              {filterVotes().map(vote => (
                <tr key={vote.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-4 flex gap-2">
                    ...
                    <Button size="sm" variant="destructive" onClick={() => deleteVote(vote.id)}>
