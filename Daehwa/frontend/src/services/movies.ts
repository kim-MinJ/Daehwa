...
export async function fetchMovieByTmdbId(tmdbId: number) {
  const res = await fetch(`http://localhost:8080/api/movies?tmdbId=${tmdbId}`);
  if (res.status === 404) return null;
  if (!res.ok) throw new Error(`Failed: ${res.status}`);
  return res.json();
}
export async function fetchMovieList(params?: { page?: number; size?: number; q?: string }) {
  ...
  const res = await fetch(`http://localhost:8080/api/movies/list?${query}`);
  if (!res.ok) throw new Error(`Failed: ${res.status}`);
...
export async function fetchCredits(id: number) {
  const res = await fetch(`http://localhost:8080/api/movies/${id}/credits`);
  if (!res.ok) throw new Error(`Failed: ${res.status}`);
...
export async function fetchMovies(query: Query = {}, token?: string) {
  ...
  // ✅ 목록 엔드포인트로 변경
  const url = `http://localhost:8080/api/movies/list?${params.toString()}`;
